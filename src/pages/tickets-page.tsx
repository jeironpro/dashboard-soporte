...
const OPCIONES_ESTADO: [TicketEstado, string][] = Object.entries(
  ETIQUETAS_ESTADO,
) as [TicketEstado, string][]

const OPCIONES_PRIORIDAD: [Prioridad, string][] = Object.entries(
  ETIQUETAS_PRIORIDAD,
) as [Prioridad, string][]

const OPCIONES_CATEGORIA: [Categoria, string][] = Object.entries(
  ETIQUETAS_CATEGORIA,
) as [Categoria, string][]
...
const COLUMNAS: ColumnDef<typeof stockFeatures, Ticket>[] = [
  {
    ...
    enableSorting: false,
    header: ({ column }) => (
      <CabeceraOrdenable etiqueta="Ticket" columna={column} />
    ),
    cell({ row }) {
  ...
  },
  {
    ...
    id: 'creado_el',
    header: ({ column }) => (
      <CabeceraOrdenable etiqueta="Creado" columna={column} />
    ),
    cell({ row }) {
      return (
  ...
  const ticketsFiltrados = useMemo(() => {
    ...
    return TICKETS.filter((ticket) => {
      ...
      return candidatos.some((valor) =>
        String(valor ?? '').toLowerCase().includes(consulta),
      )
    })
      ...
      {filas.length === 0 ? (
        <div className="relative flex flex-col items-center gap-3 overflow-hidden rounded-[var(--radius-lg)] border border-dashed border-border bg-card px-6 py-16 text-center">
          ...
          <p className="max-w-sm text-sm text-muted-foreground">
            Probá con otros términos de búsqueda o ajustá los filtros para ver
            más resultados.
          </p>
